...
struct MultiPlayerGameView: View {
    ...
    init(game: Game) {
        print("init MultiPlayerGameView")
        self.game = game
    }
    ...
    func createButton(linkedToCellAt: Int) -> some View {
        return Button(action: {
            print("clicked cell at \(linkedToCellAt)")
...
//                    Button(action: {
//                        print("clicked cell at \(0)")
//                        game.cells[0].value = XO.x
//                        game.ref?.updateChildValues(["cells": game.cells.map({ $0.toAnyObject() })])
//                    }) {
//                        Text("\(game.cells[0].value.rawValue)")
//                            .font(.largeTitle)
//                            .frame(width: 100, height: 100)
//                    }
//                    .frame(width: 100, height: 100)
//                    .border(Color.black, width: 1)
            ...
            if self.game.playerOne != "" && self.game.playerTwo == "" {
                ...
                self.observedGameId = newGame.uid
            }
            self.observeGame()
        }
    }
